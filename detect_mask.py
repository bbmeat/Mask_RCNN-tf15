...
class Mask:
    ...
    def distance(self, x, x0, y, y0, dep):
        dist = math.sqrt((x - x0) ** 2 + (y - y0) ** 2)
        dis = dep * math.tan(dist) / 10
        return dis
    ...
    def get_centor_line(self, image, depth):
        global hull, height
        # if depth == -1:
        #     height = -1
        #     volume = -1
        ...
        for c in contours:
            ...
            for i in range(len(l_x)):
                ...
                x0 = int((b1 - b) / (k2 - k1))
                y0 = int(k2 * x0 + b)
                dis = self.distance(x, x0, y, y0, depth)
                r.append(dis)
            ...
            # 计算高度
            h = self.distance(top[0], top[1], down[0], down[1], depth)
            height.append(h)
            print("height:", h)
            v = self.calc_volume(r_max, h)
            volume.append(v)
    ...
    def detect_with_h5(self, bgr_frame, depth_frame, color_dep_frame):
        ...
        global strawberry_data, label, masked_image_bw
        ...
        for i in range(N):
            ...
            if spatials['z'] == -1:
                ...
                heigh, vol = self.get_centor_line(masked_image_bw, spatials['z'])
                weight = "{:.1f}".format(vol[0] * 0.9)
                # height = "{:.1f}".format(spatials['height'])
                area = "{:.1f}".format(spatials['area'])
                # vo = "{:.1f}".format(spatials['volume'])
                spatials["heigh"] = heigh[0]
